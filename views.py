                    ...
                    if criterions.tracker.project_required == False:
                        bj, created = AccountManagerAudit.objects.update_or_create(
                            ...
                            defaults={
                                ...
                                'notice': criterions.notify,
                                'text_description': description,
                                'image_uploaded': image
                            },
                        )
                    elif criterions.tracker.project_required == True and (
                            project_id != None and project_id != '' and project_id != 'none' and project_id != '0'
                    ):
...
def ms_notification(request):
  audits = AccountManagerAudit.objects.filter(notice = True)[0:10]
  criterion= Criterion.objects.all()
  position = Position.objects.all()
  project  = Project.objects.filter(status = True)
  ...
  if request.method == "POST":
    ...
    audits = AccountManagerAudit.objects.all()[0:10]

    # Create the connector card object with the Microsoft Teams webhook URL
    myTeamsMessage = pymsteams.connectorcard("https://mmcytechcom.webhook.office.com/webhookb2/20d6c64f-8f1b-4ce7-aced-568d24228e29@0e2f1dc9-570d-4339-b890-0266f300e95c/IncomingWebhook/873f627d4aee45e4b6f504fcfd00191b/2103e32c-4d10-402b-956b-a9c4a591a9df")

    # Add a title to the card
    myTeamsMessage.title("Latest Account Manager Audits")
    myTeamsMessage.summary("Latest Account Manager Audits")

    # Loop through the audits and create sections for each
    for audit in audits:
        # Create a section for each audit
        myMessageSection = pymsteams.cardsection()

        # Add activity elements for the audit
        myMessageSection.activityTitle(f"Criterion: {audit.criterion}")
        myMessageSection.activitySubtitle(f"Project: {audit.project}")
        myMessageSection.activityText(f"Auditives: {audit.auditives}\nDate: {audit.date}\nDescription: {audit.text_description}")

        # Include the image URL in the Adaptive Card if available
        if audit.image_uploaded:
            myMessageSection.activityImage("https://www.thesoftwarereport.com/wp-content/uploads/2020/08/Cvent.jpg")
            # Add the image to the section
            myMessageSection.addImage("https://www.thesoftwarereport.com/wp-content/uploads/2020/08/Cvent.jpg", ititle="Image")



        # Add the section to the connector card
  ...
  return render(request,'notification.html',{'audits':audits,'criterion':criterion,'position':position,'project':project})